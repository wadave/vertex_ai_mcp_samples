...
class AirbnbAgent:
    ...
    def __init__(self, mcp_tools: List[Any]): # Modified to accept mcp_tools
        ...
        if not self.mcp_tools:
            logger.warning("AirbnbAgent initialized with no MCP tools. Weather search functionality may be limited.")
        else:
            logger.info(f"AirbnbAgent initialized with {len(self.mcp_tools)} MCP tools.")


    async def ainvoke(self, query: str, sessionId: str) -> dict[str, Any]:
        logger.info(f"Airbnb.ainvoke (for Weather task) called with query: '{query}', sessionId: '{sessionId}'")
        ...
        try:
            ...
            if not self.mcp_tools:
                logger.error("No MCP tools available for Airbnb.ainvoke. Cannot perform weather search.")
                return {
            ...
            logger.error(f"HTTPStatusError in Airbnb.ainvoke (Weather task): {http_err.response.status_code} - {http_err}", exc_info=True)
            return {
                'is_task_complete': True, 'require_user_input': False,
                'content': f"An error occurred with an external service for Weather task: {http_err.response.status_code}",
            }
        except Exception as e:
            logger.error(f"Unhandled exception in AirbnbAgent.ainvoke (Weather task): {type(e).__name__} - {e}", exc_info=True)
    ...
    async def stream(self, query: str, sessionId: str) -> AsyncIterable[Any]:
        logger.info(f"AirbnbAgent.stream called with query: '{query}', sessionId: '{sessionId}'")
        if not isinstance(sessionId, str) or not sessionId:
        ...
        try:
            ...
            logger.error(f"Error during AirbnbAgent.stream for session {sessionId}: {e}", exc_info=True)
