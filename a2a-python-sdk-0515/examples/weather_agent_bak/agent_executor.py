...
class WeatherAgentExecutor(BaseAgentExecutor):
    ...
    async def on_message_send(
        ...
        try:
            ...
            if task.status.message:
                task.status.message.parts = [TextPart(text=f"An internal error occurred: {e}")]
            else: # Create a new message if none exists
                from uuid import uuid4
                from a2a.types import Message, Role # Local import for brevity
                task.status.message = Message(messageId=str(uuid4()), role=Role.agent, parts=[TextPart(text=f"An internal error occurred: {e}")])
    ...
    async def on_message_stream(
        ...
        async for item in self.agent.stream(query, task.contextId): # Ensure agent.stream is async generator
